...
const UpdatePost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      const res = await fetch(`/api/v1/post/getpost?postId=${postId}`);
      const data = await res.json();
      if (!res.ok) {
        console.log(data.message);
      }
      if (res.ok) {
        const post = data.posts[0]; // Ensure you're getting the correct post
        setFormData({ title: post.title, category: post.category, image: post.image, content: post.content });
      }
    };
    fetchPost();
  }, [postId]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isImageUploaded && !formData.image) {
      setFormError("Please upload an image first");
      return;
    }
  
    try {
      const res = await fetch(`/api/v1/post/updatepost/${postId}/${currentUser._id}`, { // Ensure the correct URL and method
        method: "PUT",
        headers: {
