import React from 'react'
import { Link } from 'react-router-dom'
const PostCard = ({post}) => {
  return (
    <div className='group relative w-full border sm:w-[430px] h-[370px] overflow-hidden rounded-lg'>
       <Link to={`/post/${post.slug}`} >
        <img src={post.image} alt="cover" className='h-[256px] w-full
        object-cover group-hover:h-[200px] transition-all duration-300
        -z-10' />
       </Link>
        <div className='p-3 flex flex-col gap-2' >
            <p className='text-lg line-clamp-2 font-semibold'>{post.title}</p>
            <p className='italic text-sm'> {post.category}</p>
            <Link to={`/post/${post.slug}`}
            className='z-10 group-hover:bottom-0 absolute bottom-[-200px]
            left-0 right-0 border border-teal-500 text-teal-500
            hover:text-white transition-all duration-300 text-center
            py-2 rounded-md !rounded-tl-none m-2 hover:bg-teal-500' >
          Read article
            </Link>
        </div>
    </div>
  )
}

export default PostCard